...
import { EntityArrayResponseType, TransactionDetailsService } from '../service/transaction-details.service';
...
export class TransactionDetailsComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['id', 'key', 'value'];
  ...
  trackId = (_index: number, item: ITransactionDetails): string => this.transactionDetailsService.getTransactionDetailsIdentifier(item);

  search(query: string): void {
    if (query && TransactionDetailsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = '';
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (TransactionDetailsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.transactionDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITransactionDetails[] | null): ITransactionDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.transactionDetailsService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.transactionDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
