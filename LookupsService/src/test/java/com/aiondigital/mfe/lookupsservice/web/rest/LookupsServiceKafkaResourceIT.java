...
import com.aiondigital.mfe.lookupsservice.config.KafkaSseConsumer;
import com.aiondigital.mfe.lookupsservice.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class LookupsServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/lookups-service-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/lookups-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/lookups-service-kafka/unregister"));
                return;
            }
        }
